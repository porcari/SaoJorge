...
procedure Importar(Input: IwtsInput; Output: IwtsOutput; DataPool: IwtsDataPool);
    ...
    function importaEnderecos(EndeJarr: TJSONbase): string;
    var enderecos: IwtsWriteData;
        contato: string;
    begin
      contato := '';
      if (EndeJarr.Field['firstName'] <> nil) and (EndeJarr.Field['lastName'] <> nil) then
        contato := vartostr(EndeJarr.Field['firstName'].Value) + ' ' + vartostr(EndeJarr.Field['lastName'].Value);

      enderecos := DataPool.CreateRecordset('MILLENIUM_ECO_ACTIVE_ORACLE.PEDIDOS.ENDERECOS');
      enderecos.New;
      enderecos.SetFieldByName('LOGRADOURO', copy(AnsiUpperCase(VarToStr(EndeJarr.Field['address1'].Value)), 1, 60));
      enderecos.SetFieldByName('NUMERO', copy(AnsiUpperCase(VarToStr(EndeJarr.Field['address2'].Value)), 1, 10));
      enderecos.SetFieldByName('COMPLEMENTO', copy(AnsiUpperCase(VarToStr(EndeJarr.Field['address3'].Value)), 1, 200));
      enderecos.SetFieldByName('BAIRRO', copy(AnsiUpperCase(VarToStr(EndeJarr.Field['county'].Value)), 1, 20));
      enderecos.SetFieldByName('CIDADE', copy(RemoveAcentos(AnsiUpperCase(VarToStr(EndeJarr.Field['city'].Value))), 1, 40));
      enderecos.SetFieldByName('ESTADO', copy(AnsiUpperCase(VarToStr(EndeJarr.Field['state'].Value)), 1, 3));
      enderecos.SetFieldByName('NOME_PAIS', 'BRASIL');
      enderecos.SetFieldByName('CEP', copy(Unformat(VarToStr(EndeJarr.Field['postalCode'].Value)), 1, 9));
      enderecos.SetFieldByName('CONTATO', contato);
      enderecos.SetFieldByName('DDD', buscaDDD(EndeJarr.Field['phoneNumber'].Value));
      enderecos.SetFieldByName('FONE', buscaFone(EndeJarr.Field['phoneNumber'].Value));
      enderecos.Add;
...
begin
  ...
  try
    ...
    begin
      ...
      if jarr.Field['items'].Child[i].Field['shippingAddress'].JsonText <> 'null' then
        cliente.SetFieldByName('ENDERECO_ENTREGA', importaEnderecos(jarr.Field['items'].Child[i].Field['shippingAddress']));
      if jarr.Field['items'].Child[i].Field['billingAddress'].JsonText <> 'null' then
        cliente.SetFieldByName('ENDERECO_COBRANCA', importaEnderecos(jarr.Field['items'].Child[i].Field['billingAddress']));
      cliente.Add;
    end;
  finally
    FreeAndNil(jarr);
  ...
  try
    ...
    Call('MILLENIUM_ECO.CLIENTES.INCLUIROUALTERAR', cliente, true, '', urlRest);
    ...
    on E: Exception do
    begin
      raise Exception.Create('Erro ao importar cliente ' + cliente.GetFieldAsString('NOME') + ' ' + E.Message);
    end;
  end;
end;
