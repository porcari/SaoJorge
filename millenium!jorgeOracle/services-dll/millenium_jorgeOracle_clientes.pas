...
var cmd01: IwtsCommand;
    cliente,enderecos,enderecosDefault,vitOracle,result: IwtsWriteData;
    jarr: TJSONarray;
    token,sres,urlBase,ddd,fone,nome,contato,json,idCliente: string;
    i,x,vitrine:Integer;
begin
  try
    ...
    sres := RestClientCenter(token,'Get',urlBase,PChar('/ccadmin/v1/profiles/?q=email co "'+RemoveEspacos(VarToStr(Input.GetParamByName('EMAIL'))) + '"'));
    ...
    try
      ...
      begin
        ...
        begin
          ...
          enderecos.SetFieldByName('CONTATO', copy(contato, 1, 60));
        ...
        try
          result := Call('MILLENIUM_ECO.CLIENTES.INCLUIROUALTERAR', cliente, true, '','');
          ...
          begin
            ...
            try
              RestClientCenter(token,'Put',urlBase,PChar('/ccadmin/v1/profiles/'+idCliente),json);
...
procedure ImportarParaMillennium(Input: IwtsInput; Output: IwtsOutput; DataPool: IwtsDataPool);
  ...
  try
    ...
    try
      ...
      begin
        ...
        result := Call('MILLENIUM_ECO.CLIENTES.INCLUIROUALTERAR', cliente, true, '','');
        Output.NewRecord;
        Output.Values['CLIENTE'] := result.GetFieldByName('CLIENTE');
      end;
    finally
      FreeAndNil(jarr);
    end;
...
var cmd01: IwtsCommand;
    cliente,vitOracle: IwtsWriteData;
    jarr: TJSONarray;
    token,sres,urlBase,nome: string;
    i,pag: Integer;
begin
  ...
  if Input.GetParamAsString('EMAIL')<>'' then
    sres := RestClientCenter(token,'Get',urlBase,PChar('/ccadmin/v1/profiles?'+
...
begin
  ...
  try
    for i := 0 to jarr.Field['items'].Count - 1 do
    begin
      DataPool.BeginSavePoint(Pchar('ORC_CLIENTES_'+vartostr(jarr.Field['items'].Child[i].Field['id'].Value)));
      try
        ...
        on E: Exception do
        begin
          DataPool.RollBackToSavePoint(Pchar('ORC_CLIENTES_'+vartostr(jarr.Field['items'].Child[i].Field['id'].Value)));
...
var cmd01: IwtsCommand;
    vitOracle: IwtsWriteData;
    token,urlBase,json,id: string;
begin
  cmd01 := DataPool.Open('millenium');
  ...
  try        
    RestClientCenter(token,'Put',urlBase,PChar('/ccadmin/v1/profiles/'+id),json);
  except
    on E: Exception do
    begin
      raise exception.Create('Erro ao marcar cliente como atacado ' + E.Message);
    end;
  end; 
end;
...
initialization
  ...
  wtsRegisterProc('CLIENTES.ImportarParaMillennium', ImportarParaMillennium);
